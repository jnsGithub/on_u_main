          ...
          builder: (context, snapshot) =>
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  Duration difference = snapshot.data!.docs[index].data()['createDate'].toDate().difference(DateTime.now()); // 시간, 분, 초를 나눠서 출력
                  ...
                  return Padding(
                    padding: snapshot.data!.docs[index].data()['senderId'] != uid ? EdgeInsets.only(left: 10) : EdgeInsets.only(right: 10),
                    child: Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        snapshot.data!.docs[index].data()['senderId'] != uid ? Row(
                          children: [
                            Container(
                              width: 50,
                              height: 50,
                        ...
                        Expanded(
                          child: Column(
                            crossAxisAlignment: snapshot.data!.docs[index].data()['senderId'] != uid ? CrossAxisAlignment.start : CrossAxisAlignment.end,
                            children: [
                              snapshot.data!.docs[index].data()['senderId'] != uid ? Text(controller.chatRoom.counselorName, style: TextStyle(fontSize: 14, color: gray700, fontWeight: FontWeight.w400),) : Container(),
                              SizedBox(height: 8,),
                              ChatBubble(
                                clipper: ChatBubbleClipper5(type: snapshot.data!.docs[index].data()['senderId'] != uid ? BubbleType.receiverBubble : BubbleType.sendBubble),
                                alignment: snapshot.data!.docs[index].data()['senderId'] != uid ? Alignment.topLeft : Alignment.topRight,
                                backGroundColor: snapshot.data!.docs[index].data()['senderId'] != uid ? bgColor : Color(0xffE8F9F4),
                                child: Linkify(
                                  onOpen: (link) async {
                              ...
                              SizedBox(height: 8,),
                              Text(hours > 24 ? '${(hours/24).toInt()}일 전' : hours > 0 && hours < 24 ? '${hours}시간 전' : minutes > 0 ? '${minutes}분 전' : '${seconds}초 전', style: TextStyle(fontSize: 14, color: gray400, fontWeight: FontWeight.w400),),
                            ],
                          ),
                        ),
                      ],
      ...
      bottomNavigationBar: Container(
        ...
        child: SafeArea(
          child: Row(
            ...
            children: [
              IconButton(onPressed: (){ // TODO: 사진 불러오기 해야됨.
                    ...
                    IconButton(
                      ...
                      onPressed: () async{
                        if(controller.chatController.text != ''){
                          if(await controller.sendMsg(controller.chatRoom.documentId, controller.chatController.text, uid!)){
